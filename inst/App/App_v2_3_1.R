...
packages=c("shiny","shinyjs", "RSQLite", "ggplot2",
           ...
           "shinydashboard","data.table","dbplyr","shinyWidgets","DT")
...
csv_chunk_multiple<-function(file_input,chunk_size,file_output,var_sheet,var_details,db_name,type){
  ...
  if( type== ".sqlite"){
    initial_recoded<-rec_with_table(read.csv(csv_files[1],nrows=1,sep="|"),variables =var_sheet, variable_details = var_details,database_name =db_name)
    db_connection<-dbConnect(RSQLite::SQLite(),dbname=file_output)
  }
  for (i in 1:length(csv_files)){
    if(type==".sqlite"){
      dbWriteTable(conn = db_connection, name =paste0(tools::file_path_sans_ext(basename(csv_files[i])),"_recoded"), value = initial_recoded, overwrite=FALSE)
      dbExecute(conn = db_connection, paste0("DELETE from ",paste0(tools::file_path_sans_ext(basename(csv_files[i])),"_recoded")))
    }
    ...
    while (!end) {
      ...
      if(n==1){
        chunk <- read.csv(input_conn, nrows = chunk_size ,header = TRUE,sep="|", colClasses = "character")
      }
      else{
        chunk <- read.csv(input_conn, nrows = chunk_size ,header = FALSE,sep="|",colClasses = "character")
      }
      ...
      if(type==".sqlite"){
        dbAppendTable(conn = db_connection,name =as.character(paste0(tools::file_path_sans_ext(basename(csv_files[i])),"_recoded")), value = recoded )
      }
      if (n==ceiling((num_rows-1)/chunk_size)+1) {
        end <- TRUE
      }
      
      incProgress(1/ceiling((num_rows-1)/chunk_size)*(1/length(csv_files))) 
    }
...
csv_chunk<-function(file_input,chunk_size,file_output,var_sheet,var_details,db_name,var){
  withProgress(message = "Please wait while your dataset is being recoded!", value = 0,{
    ...
    while (!end) {
      ...
      incProgress(1/ceiling((num_rows-1)/chunk_size)) 
    }
    # Close both input and output files
    closeAllConnections()
...
ui <- fluidPage(
  ...
  tags$head(
    tags$style(HTML("
      ...
      .marquee {
        ...
        body > .container-fluid {  /* Assuming you are using a fluid page layout */
                margin-bottom: 50px;  /* Adjust this value based on the height of your marquee */
            }
  ...
  div(class = "marquee",
      div(class = "marquee-content",
          ...
          HTML('For inquiries or questions, please write to: ',
      ...
      tabsetPanel(
        ...
        tabPanel("Recodeflow", 
                 ...
                 conditionalPanel(
                   ...
                   radioButtons("typeoutput", "Output format?", choices = c(".sqlite", ".csv"), selected = ".sqlite")
                 ),
                 ...
                 conditionalPanel(
                   condition = "input.singleorgroup == 'Multiple' & input.importType == 'Large pipe-delimited CSVs'",
                   textInput("pathtofolderwrite", "Please enter the path to the folder/SQLite database where all recoded files will be stored", value = "", placeholder = "e.g. C:/users/../files/recoded_data OR C:/users/../files/recoded_data/test.sqlite", width = 500)
                 ),
                 uiOutput("import"),
                 tableOutput("test"),
        ...
        tabPanel("Summary", 
                 ...
                 fluidRow(
                   column(3,verbatimTextOutput("levels"),
                   verbatimTextOutput("DistinctVals")),
                   column(3, DTOutput("freq_table"))
                 )
...
server <- function(input, output, session) {
  ...
  output$dvlpath1<-renderUI({
    ...
    if(input$derived=="Yes"){
      if(input$library=="Yes"){
        ...
        textInput("dvlpath","Please enter the path to the DVL", placeholder = "e.g. C:/users/../files/Derived_variable_libraries.csv")
    ...
    if(input$derived=="Yes"){
      if(input$library=="Yes"){
        ...
        selectizeInput("dvllist","please select the derived variables of your interest",
                       choices = unique(read.csv(input$dvlpath)[,"variable_name"]),
  ...
  df_recoded<- eventReactive(input$recode,{
    ...
    if(input$importType%in% c("Comma-delimited CSVs",".RDS",".sas7bdat")){
      ...
      csv_chunk_multiple(file_input = input$pathtofolder, file_output =input$pathtofolderwrite ,chunk_size =input$chunksize1, var_sheet = finalV(), var_details =finalD(), db_name = input$db, type = input$typeoutput )
    }
