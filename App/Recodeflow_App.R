...
ui <- fluidPage(
  ...
  titlePanel("Data Curation Platform"),
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      uiOutput("typeEnd"),
      uiOutput("cont_miss"),
      uiOutput("cont_mis_min"),
      uiOutput("cont_mis_max"),
      uiOutput("categories"),
      uiOutput("catS"),
      uiOutput("catF"),
...
server <- function(input, output, session) {
 ...
 # output$upV<-renderUI({
  ...
  output$downloadrecoded<-downloadHandler(
    filename= function() { paste0("recoded",Sys.time(),ifelse(input$downloadType1==".csv",".csv",".RDS"))
 ...
 observeEvent(input$func_code,{
  ...
  output$download_data <- 
    downloadHandler(
      filename= function() { paste0("details_sheet",Sys.time(),".csv")
  ...
  df_recoded<- eventReactive(input$recode,{
    ...
    if(input$importType%in% c(".csv",".RDS",".sas7bdat")){
     tryCatch(withProgress(  rec_with_table(dataframe(),variables =finalV(), variable_details =finalD(),database_name =input$db), message = "Please wait!"), error=function(e){
