...
### multiple csv pipe delimited files 

csv_chunk_multiple<-function(file_input,chunk_size,var_sheet,var_details,db_name,var){
  ...
  for (i in 1:length(csv_files)){
    ...
    output_file <- paste0(sub("\\.\\w+$", "", csv_files[i]),"_recoded.csv")
  ...
  dashboardPage(
    ...
    ')),
      ...
      tabsetPanel(
        ...
        tabPanel("Recodeflow", 
                 radioButtons("importType", "Please choose the format of your original dataset" ,choices = c(".csv",".RDS",".sas7bdat",".SQLite", "pipe delimited (large datasets)"), selected = ".csv"),
                 conditionalPanel(
                   condition = "input.importType == 'pipe delimited (large datasets)'",
                 ...
                 conditionalPanel(
                   ...
                   textInput("pathtofolder", "Please enter the path to the folder where all files are located (recoded files will be stored in the same location)", value = "", placeholder = "e.g. C:/users/../files")
                 ),
...
server <- function(input, output, session) {
  ...
  output$path<-renderUI({
    if((input$importType == ".SQLite") | (input$importType== "pipe delimited (large datasets)"& input$singleorgroup=="Single" ) ){
  ...
  output$pathtowrite<-renderUI({
    if(input$importType=="pipe delimited (large datasets)" & input$singleorgroup=="Single"){
  ...
  output$chunksize<-renderUI({
    if(input$importType %in% c(".SQLite","pipe delimited (large datasets)")){
  ...
  names<-reactive({
    ...
    else if(input$importType=="pipe delimited (large datasets)" & input$singleorgroup=="Single") {
      colnames(read.csv(input$path1,nrows=1,sep="|"))
    }
    
    else if(input$importType=="pipe delimited (large datasets)" & input$singleorgroup=="Multiple") {
  ...
  df_recoded<- eventReactive(input$recode,{
    ...
    if(input$importType%in% c(".csv",".RDS",".sas7bdat")){
      ...
      csv_chunk_multiple(file_input = input$pathtofolder, chunk_size =input$chunksize1, var_sheet = finalV(), var_details =finalD(), db_name = input$db, var=input$var )
    }
